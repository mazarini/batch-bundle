...
use Mazarini\BatchBundle\Reader\Database\DoctrineReader;
...
#[AsCommand(
    name: 'app:read-persons-doctrine',
    description: 'Read persons from SQLite database using Doctrine DBAL'
)]
class ReadPersonsDoctrineCommand extends Command
{
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $io = new SymfonyStyle($input, $output);

        $io->title('Reading Persons from SQLite Database (Doctrine DBAL)');
        ...
        // Configure reader
        $sql    = 'SELECT id, first_name, last_name, email, age, is_active, salary FROM persons WHERE age > :min_age ORDER BY id';
        $reader = new DoctrineReader($connection, $sql, ['min_age' => 25]);
        $reader->configure($dataCollection, ['id', 'first_name', 'last_name', 'email', 'age', 'is_active', 'salary']);

        $io->section('Person Records (Age > 25):');
        ...
        $io->success("Successfully read {$count} person records using Doctrine DBAL!");
